...
(*
    ...
    let read_back ~monad_ops ~config ~fn ~ptr =
...
module Pcache_layers = struct
  ...
  type ('k,'v,'r,'t,'blk,'pl_data,'pl_internal_state,'pcl_internal_state,'fd,'e) pcache_layers = {
    ...
    pl_state_ops  : ('pl_data,'r,'pl_internal_state)pl_state_ops;
    ...
    with_dmap     : (('r,'k,'v)dmap_state,'t) with_state option ref;

    (* FIXME why does dmap_ops need pl write_node? *)
    dmap_ops      : write_node:('pl_internal_state -> (unit,'t)m) ->  ('k,'v,'r,'t)dmap_ops
  }
